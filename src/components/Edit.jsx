...
const Edit = () => {
  ...
  const [oldFiles, setOldFiles] = useState({
    oldFoodMenuFile: null,
    oldDrinkMenuFile: null,
    oldSpaMenuFile: null,
  });

  const fetchOldFileData = async (url) => {
    if (!url) return null;
    try {
      const response = await axios.head(url);
      return {
        name: url.split("menu_uploads/").pop(),
  ...
  useEffect(() => {
    ...
    const loadFiles = async () => {
      const [food, drink, spa] = await Promise.all([
        fetchOldFileData(editData.foodMenuFileUrl),
        fetchOldFileData(editData.drinkMenuFileUrl),
        fetchOldFileData(editData.spaMenuFileUrl),
      ]);

      setOldFiles({ oldFoodMenuFile: food, oldDrinkMenuFile: drink, oldSpaMenuFile: spa });
    };
  ...
  useEffect(() => {
    if (oldFiles.oldFoodMenuFile) {
      setFoodStatus("completed");
      setFoodProgress(100);
    }
    if (oldFiles.oldDrinkMenuFile) {
      setDrinkStatus("completed");
      setDrinkProgress(100);
    }
    if (oldFiles.oldSpaMenuFile) {
      setSpaStatus("completed");
      setSpaProgress(100);
    }
  }, [oldFiles, setFoodProgress, setFoodStatus, setDrinkProgress, setDrinkStatus, setSpaProgress, setSpaStatus]);
  ...
  const uploadsInProgress = foodStatus === "uploading" || drinkStatus === "uploading" || spaStatus === "uploading";
  const isSaveDisabled = uploadsInProgress;
  ...
  return (
    <div className="fixed flex items-center justify-center z-50 inset-0 bg-[#34405499]/60 backdrop-blur-[2px]">
      <div className="bg-white w-[644px] mx-8 rounded-xl flex flex-col gap-2 p-6">
        <div className="border-b border-gray-200 px-5">
          <div className=" mb-4 ">
            <div className="flex items-center justify-between py-6">
              <p className="font-semibold text-2xl text-gray-900">Edit '{editData?.name}'</p>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-8 pt-6 pb-8 px-5">
          <div className="flex flex-col gap-8 overflow-y-auto h-100 scrollbar-thin scrollbar-right scrollbar-thumb-red-400 scrollbar-track-gray-50 scrollbar-thumb-inset text-gray-100">
            ...
            <div className="flex flex-col gap-3 w-full">
              <label htmlFor="menuName" className="text-base text-[#101828]">Menu Name</label>
              <input
                type="text"
                id="menuName"
                {...register("menuName", { required: true, minLength: 3, maxLength: 50 })}
            ...
            <div className="flex flex-col gap-6 pb-5 border-b-3 border-gray-100">
              <div className="flex items-center gap-6">
                <p className="text-[#101828] text-base font-medium flex-1">Restaurant Menus</p>
                <div onClick={() => toggleExpand("restaurant")} className="cursor-pointer">
                  {expanded.includes("restaurant") ? <Minus size={24} className="text-gray-900" /> : <Plus size={24} className="text-gray-900" />}
                </div>
              </div>
              ...
              {expanded.includes("restaurant") && (
                <div className="flex items-center justify-center flex-col md:flex-row gap-6">
                  <MenuUpload
                    ...
                    file={foodMenuFile || oldFiles.oldFoodMenuFile}
                  ...
                  <MenuUpload
                    ...
                    file={drinkMenuFile || oldFiles.oldDrinkMenuFile}
            ...
            <div className="flex flex-col gap-6 pb-5 border-b-3 border-gray-100">
              <div className="flex items-center gap-6">
                <p className="text-[#101828] text-base font-medium flex-1">Spa Menu</p>
                <div onClick={() => toggleExpand("spa")} className="cursor-pointer">
                  {expanded.includes("spa") ? <Minus size={24} className="text-gray-900" /> : <Plus size={24} className="text-gray-900" />}
                </div>
              </div>
              ...
              {expanded.includes("spa") && (
                <div className="flex items-center justify-center gap-6">
                  <MenuUpload
                    ...
                    file={spaMenuFile || oldFiles.oldSpaMenuFile}
            ...
            <div className="flex flex-col gap-6 pb-5 border-b-3 border-gray-100">
              <div className="flex items-center gap-6">
                <p className="text-[#101828] text-base font-medium flex-1">Review Link</p>
                <div onClick={() => toggleExpand("review")} className="cursor-pointer">
                  {expanded.includes("review") ? <Minus size={24} className="text-gray-900" /> : <Plus size={24} className="text-gray-900" />}
                </div>
              </div>
          ...
          <div className="flex items-center w-full gap-3">
            ...
            <button
              ...
              className={`cursor-pointer w-full text-base font-semibold text-white flex items-center justify-center rounded-lg py-3 px-7 ${
